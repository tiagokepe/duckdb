...
virtual string BenchmarkInfo() {
	return StringUtil::Format(
	    "Runs the following query : SELECT w1.doc AS doc1, w2.doc as doc2, COUNT(*) AS c FROM words AS w1 JOIN words "
	    "AS w2 ON (w1.word=w2.word) GROUP BY doc1, doc2 ORDER BY c DESC LIMIT 10 without indexes.");
}
...
virtual string BenchmarkInfo() {
	return StringUtil::Format(
	    "Runs the following query : SELECT w1.doc AS doc1, w2.doc as doc2, COUNT(*) AS c FROM words AS w1 JOIN words "
	    "AS w2 ON (w1.word=w2.word) GROUP BY doc1, doc2 ORDER BY c DESC LIMIT 10 with an index on words.word.");
}
