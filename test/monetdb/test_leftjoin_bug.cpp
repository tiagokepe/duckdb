...
TEST_CASE("MonetDB Test: null_matches_in_outer.Bug-6398.sql", "[monetdb]") {
    ...
    REQUIRE_NO_FAIL(con.Query("create table A (A bigint, B varchar);"));

    REQUIRE_NO_FAIL(con.Query("insert into A values (0, 'zero');"));
    REQUIRE_NO_FAIL(con.Query("insert into A  values (1, 'one');"));
    REQUIRE_NO_FAIL(con.Query("insert into A  values (2, 'two');"));
    REQUIRE_NO_FAIL(con.Query("insert into A  values (null, null);"));

    REQUIRE_NO_FAIL(con.Query("create table B (A bigint, B varchar);"));

    REQUIRE_NO_FAIL(con.Query("insert into B values (2, 'due');"));
    REQUIRE_NO_FAIL(con.Query("insert into B values (4, 'quattro');"));
    REQUIRE_NO_FAIL(con.Query("insert into B values (null, 'this is not null')"));

    result = con.Query("select * from A left outer join B on A.A = B.A or "
                       "(A.A is null and B.A is null) ORDER BY 1;");
